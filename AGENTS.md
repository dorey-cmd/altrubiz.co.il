...
- **Mobile First-Class Knowledge Navigation Experience**: Compact vertical rhythm, persistent mobile orientation via bottom-floating pill (`scrollY > 200`) showing current section and reading progress `%`, mobile-native drawer with one-tap section jumping and explicit one-tap parent Hub bridge, bounded touch modals (`max-h-[90vh]`), and inverted header hierarchy (keeping H1 dominant above the fold).
- **Persistent Desktop Navigation, Sticky Integrity & Scroll Sovereignty**: Architectural Invariant: *"Navigation follows the reader - never the reverse."* Reader's document scroll position is sovereign. Passive active-section tracking may ONLY scroll the sidebar's own internal TOC container (`container.scrollTo`), never calling `scrollIntoView()` which scrolls the document or window ancestors. Desktop sidebar relies on native CSS `position: sticky` (containing block ancestors must never set `overflow: hidden`, `overflow-x: hidden`, or `overflow-y: hidden`; use `overflow-x: clip` on `html, body`). The sidebar sizes naturally to its content (`max-h-[calc(100vh-8.5rem)]`) with zero artificial blank "legroom", and internal TOC scrolling (`max-h-[46vh] xl:max-h-[50vh]`) for long lists.
