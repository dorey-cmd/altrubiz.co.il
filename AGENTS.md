...
- **Persistent Desktop Navigation & Scroll Sovereignty**: Architectural Invariant: *"Navigation follows the reader - never the reverse."* Reader's document scroll position is sovereign. Passive active-section tracking may ONLY scroll the sidebar's own internal TOC container (`container.scrollTo`), never calling `scrollIntoView()` which scrolls the document or window ancestors. Desktop sidebar is bounded strictly to viewport height (`h-[calc(100vh-7.5rem)]`) with non-overflowing compact secondary CTA card.
